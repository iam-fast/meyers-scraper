...
def register_meyers_tools(mcp: FastMCP):
    ...
    async def get_all_menus(
        school_id: Annotated[str, Field(
            default=SCHOOL_ID,
            description="School ID for the Meyers API"
        )] = SCHOOL_ID,
        language: Annotated[str, Field(
            default=DEFAULT_LANGUAGE,
            description="Language code (e.g., 'en', 'de')"
        )] = DEFAULT_LANGUAGE,
        target_offer_id: Annotated[str, Field(
        ...
        """
        try:
            logger.info(f"Fetching all menus for school_id: {school_id}, language: {language}, target_offer_id: {target_offer_id}")
    ...
    async def get_menu_by_date(
        ...
        """
        try:
            logger.info(f"Fetching menu for date: {date}, school_id: {school_id}, language: {language}, target_offer_id: {target_offer_id}")
